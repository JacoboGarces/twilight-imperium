...
{
    public class AddSpecialCardUseCaseTests
    {
        private readonly Mock<IEventRepository> _mockRepository;
        private readonly AddSpecialCardUseCase _useCase;

        public AddSpecialCardUseCaseTests()
        {
            _mockRepository = new Mock<IEventRepository>();
            _useCase = new AddSpecialCardUseCase( _mockRepository.Object );
        }

        [Fact]
        public void Execute_Success()
        {
            var command = new AddSpecialCardCommand( EnumCard.DEFENSE_PLUS, "battle_id" );
            ...
            Assert.IsType<SpecialCardAdded>( result[0] );
        ...
        public void Execute_Success_Event()
        {
            ...
            var valueResult = (SpecialCardAdded)result[0];
            Assert.Equal( 1, result.Count );
            Assert.Equal( EnumCard.DEFENSE_PLUS, valueResult.SpecialCard );
            Assert.Equal( addSpecialCard.Type, result[0].Type );
            Assert.IsType<SpecialCardAdded>( result[0] );
            _mockRepository.Verify( repo => repo.FindByAggregateId( "battle_id" ), Times.Once );
            _mockRepository.Verify( repo => repo.Save( It.IsAny<DomainEvent>() ), Times.AtLeastOnce );
        }
