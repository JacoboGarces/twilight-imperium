...
{
    public class ApplyEffectSpecialCardUseCaseTests
    {
        private readonly Mock<IEventRepository> _mockRepository;
        private readonly ApplyEffectSpecialCardUseCase _useCase;

        public ApplyEffectSpecialCardUseCaseTests()
        {
            _mockRepository = new Mock<IEventRepository>();
            _useCase = new ApplyEffectSpecialCardUseCase( _mockRepository.Object );
        }

        [Fact]
        public void Execute_Success()
        {
            var command = new ApplyEffectSpecialCardCommand( "battle_id" );
            ...
            Assert.IsType<SpecialCardEffectApplied>( result[0] );
        ...
        public void Execute_Success_Event()
        {
            ...
            var addSpecialCard = new SpecialCardAdded( EnumCard.DEFENSE_PLUS );

            var applySpecialCard = new SpecialCardEffectApplied();
            ...
            domainEvents.Add( addSpecialCard );
            domainEvents.Add( applySpecialCard );
            ...
            var valueResult = (SpecialCardEffectApplied)result[0];
            Assert.Equal( 1, result.Count );
            Assert.Equal( applySpecialCard.Type, result[0].Type );
            Assert.IsType<SpecialCardEffectApplied>( result[0] );
            _mockRepository.Verify( repo => repo.FindByAggregateId( "battle_id" ), Times.Once );
            _mockRepository.Verify( repo => repo.Save( It.IsAny<DomainEvent>() ), Times.AtLeastOnce );
        }
