...
{
    public class SurrenderUseCaseTests
    {
        private readonly Mock<IEventRepository> _mockRepository;
        private readonly SurrenderUseCase _useCase;

        public SurrenderUseCaseTests()
        {
            _mockRepository = new Mock<IEventRepository>();
            _useCase = new SurrenderUseCase( _mockRepository.Object );
        }

        [Fact]
        public void Execute_Success()
        {
            var command = new SurrenderCommand( "battle_id" );
            ...
            Assert.IsType<PlayerSurrendered>( result[0] );
        ...
        public void Execute_Success_Event()
        {
            ...
            var surrenderPlayer = new PlayerSurrendered();
            ...
            domainEvents.Add( surrenderPlayer );
            ...
            var valueResult = (PlayerSurrendered)result[0];
            Assert.Equal( 1, result.Count );
            Assert.Equal( surrenderPlayer.Type, result[0].Type );
            Assert.IsType<PlayerSurrendered>( result[0] );
            _mockRepository.Verify( repo => repo.FindByAggregateId( "battle_id" ), Times.Once );
            _mockRepository.Verify( repo => repo.Save( It.IsAny<DomainEvent>() ), Times.AtLeastOnce );
        }
